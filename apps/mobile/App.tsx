...
import type { BetBuilderValidation, BetSlip, CashoutQuote, CasinoGame, CasinoRound, CommunityPost, ContentArticle, ContentKind, DemoProfile, DemoSession, DemoTicket, DetailedMarket, EventDetailResponse, LedgerEntry, LottoDraw, LottoEntry, OfferLeague, OfferResponse, OfferTimeFilter, Promotion, ResultsDashboard, SlipMode, SportsEvent, VirtualEvent, VirtualResult, Wallet } from '@feg/contracts';
...
function App() {
  ...
  const [virtualsOpen, setVirtualsOpen] = useState(false);
  ...
  const [results, setResults] = useState<ResultsDashboard | null>(null);
  ...
  const openDiscovery = async (route: 'promotions' | ContentKind) => {
    setDiscoveryOpen(route);
    try { route === 'promotions' ? setPromotions(await api.getPromotions()) : setContent(await api.getContent(route)); }
  ...
  const openVirtuals = async () => { setVirtualsOpen(true); try { setVirtualEvents(await api.getVirtualEvents()); } catch (error) { Alert.alert('Virtual games', error instanceof Error ? error.message : 'Unable to load virtual games.'); } };
  const openResults = async () => { setResultsOpen(true); try { setResults(await api.getResults()); } catch (error) { Alert.alert('Results', error instanceof Error ? error.message : 'Unable to load results.'); } };
  ...
  return (
    <SafeAreaProvider>
      <SafeAreaView edges={['top']} style={styles.screen}>
        ...
        <BottomNavigation active={activeTab} onChange={tab => { setActiveTab(tab); setDetailEvent(null); if (tab !== 'Menu') { setProfileOpen(false); setWalletOpen(false); setDiscoveryOpen(null); setCommunityOpen(false); setLottoOpen(false); setVirtualsOpen(false); setResultsOpen(false); } }} />
      </SafeAreaView>
    </SafeAreaProvider>
  );
}
...
function MenuScreen({ onProfile, onDiscovery, onCommunity, onLotto, onVirtuals, onResults }: { onProfile: () => void; onDiscovery: (route: 'promotions' | ContentKind) => void; onCommunity: () => void; onLotto: () => void; onVirtuals: () => void; onResults: () => void }) {
  ...
  return <View testID="menu-screen">
    ...
    {['Profile & demo wallet', 'Promotions', 'Community', 'Lotto', 'Virtual games', 'Results & stats', 'News', 'Help centre', 'Responsible play', 'Settings'].map(item =>
      <Pressable key={item} testID={item === 'Profile & demo wallet' ? 'open-profile-button' : item === 'Community' ? 'open-community-button' : item === 'Lotto' ? 'open-lotto-button' : item === 'Virtual games' ? 'open-virtuals-button' : item === 'Results & stats' ? 'open-results-button' : routes[item] ? `open-${routes[item]}-button` : undefined} onPress={item === 'Profile & demo wallet' ? onProfile : item === 'Community' ? onCommunity : item === 'Lotto' ? onLotto : item === 'Virtual games' ? onVirtuals : item === 'Results & stats' ? onResults : routes[item] ? () => onDiscovery(routes[item]!) : undefined} style={styles.menuRow}><Text style={styles.menuText}>{item}</Text><Text style={styles.chevron}>›</Text></Pressable>)}
  </View>;
}
...
function VirtualsScreen({ events, result, onBack, onPlay }: { events: VirtualEvent[]; result: VirtualResult | null; onBack: () => void; onPlay: (eventId: string) => Promise<void> }) {
  const [busy, setBusy] = useState<string | null>(null);
  return <View testID="virtuals-screen"><View style={styles.screenHeading}><Pressable testID="virtuals-back" onPress={onBack}><Text style={styles.backText}>‹</Text></Pressable><Text style={styles.screenTitle}>VIRTUAL GAMES</Text><Text style={styles.ticketStatus}>SIMULATION</Text></View><View style={styles.casinoDisclosure}><Text style={styles.slipWarningCode}>DETERMINISTIC · NO CASH VALUE</Text><Text style={styles.infoBody}>Each launch advances a reproducible simulated round. No stake or payout is involved.</Text></View>{events.map(event => <View key={event.id} testID={`virtual-${event.sport}`} style={styles.discoveryCard}><View style={styles.gameTileTop}><Text style={styles.gameBadge}>{event.sport.toUpperCase()}</Text><Text style={styles.ticketStatus}>ROUND {event.round}</Text></View><Text style={styles.discoveryTitle}>{event.home} · {event.away}</Text><Text style={styles.discoverySummary}>Next simulation in {event.startsInSeconds}s</Text><Pressable testID={`play-virtual-${event.sport}`} disabled={busy !== null} onPress={async () => { setBusy(event.id); try { await onPlay(event.id); } finally { setBusy(null); } }} style={[styles.walletButton, busy !== null && styles.disabled]}><Text style={styles.placeButtonText}>{busy === event.id ? 'SIMULATING…' : 'PLAY ROUND'}</Text></Pressable></View>)}{result && <View testID="virtual-result" style={styles.casinoStage}><Text style={styles.casinoStageLabel}>{result.sport.toUpperCase()} RESULT</Text><Text style={styles.casinoResult}>{result.homeScore} – {result.awayScore}</Text><Text style={styles.casinoOutcome}>{result.outcome}</Text><Text style={styles.ticketStatus}>ROUND {result.round}</Text></View>}</View>;
}

function ResultsScreen({ dashboard, onBack, onFavorite }: { dashboard: ResultsDashboard | null; onBack: () => void; onFavorite: (competitionId: string) => Promise<void> }) {
  return <View testID="results-screen"><View style={styles.screenHeading}><Pressable testID="results-back" onPress={onBack}><Text style={styles.backText}>‹</Text></Pressable><Text style={styles.screenTitle}>RESULTS & STATS</Text></View>{!dashboard ? <OfferSkeleton /> : <><Text style={styles.groupHeading}>FINISHED FIXTURES</Text>{dashboard.fixtures.map(event => <View key={event.id} style={styles.sessionRow}><View><Text style={styles.menuText}>{event.home} · {event.away}</Text><Text style={styles.ledgerDate}>{event.league}</Text></View><Text style={styles.discoveryTitle}>{event.score ?? 'FT'}</Text></View>)}<Text style={styles.groupHeading}>VIRTUAL HISTORY</Text>{dashboard.virtual.map(result => <View key={result.id} style={styles.sessionRow}><Text style={styles.menuText}>{result.home} · {result.away}</Text><Text style={styles.ticketStatus}>{result.homeScore} – {result.awayScore}</Text></View>)}{!dashboard.virtual.length && <Text style={styles.emptyBody}>Play a virtual round to create history.</Text>}<Text style={styles.groupHeading}>TABLES</Text>{dashboard.tables.map(table => <View key={table.id} testID={`competition-${table.id}`} style={styles.discoveryCard}><View style={styles.gameTileTop}><Text style={styles.discoveryTitle}>{table.name}</Text><Pressable testID={`favorite-${table.id}`} accessibilityLabel={`${table.favorite ? 'Remove' : 'Add'} ${table.name} favorite`} onPress={() => onFavorite(table.id)}><Text style={[styles.detailFavorite, table.favorite && styles.eventFavoriteActive]}>★</Text></Pressable></View>{table.rows.map(row => <View key={row.team} style={styles.tableRow}><Text style={styles.infoBody}>{row.position}. {row.team}</Text><Text style={styles.ticketStatus}>{row.played}P · {row.points}PTS</Text></View>)}</View>)}</>}</View>;
}
...
function DiscoveryScreen({ route, promotions, content, onBack, onOptIn }: { route: 'promotions' | ContentKind; promotions: Promotion[]; content: ContentArticle[]; onBack: () => void; onOptIn: (promotionId: string) => Promise<void> }) {
  const [article, setArticle] = useState<ContentArticle | null>(null);
  const title = route === 'promotions' ? 'PROMOTIONS' : route === 'responsible_play' ? 'RESPONSIBLE PLAY' : route === 'help' ? 'HELP CENTRE' : route.toUpperCase();
  if (article) return <View testID="article-screen"><View style={styles.screenHeading}><Pressable testID="article-back" onPress={() => setArticle(null)}><Text style={styles.backText}>‹</Text></Pressable><Text style={styles.screenTitle}>{article.kind.replace('_', ' ').toUpperCase()}</Text></View><View style={styles.articleCard}><Text style={styles.discoveryTitle}>{article.title}</Text><Text style={styles.discoverySummary}>{article.summary}</Text>{article.body.map(paragraph => <Text key={paragraph} style={styles.infoBody}>{paragraph}</Text>)}</View></View>;
  return <View testID="discovery-screen"><View style={styles.screenHeading}><Pressable testID="discovery-back" onPress={onBack}><Text style={styles.backText}>‹</Text></Pressable><Text style={styles.screenTitle}>{title}</Text><Text style={styles.ticketStatus}>{route === 'promotions' ? 'DEMO ONLY' : 'FEG GUIDE'}</Text></View>
    {route === 'promotions' ? promotions.map(item => <View key={item.id} testID={`promotion-${item.id}`} style={styles.discoveryCard}><View style={styles.gameTileTop}><Text style={styles.gameBadge}>{item.rewardLabel}</Text><Text style={styles.ticketStatus}>{item.optedIn ? 'JOINED' : 'ELIGIBLE'}</Text></View><Text style={styles.discoveryTitle}>{item.title}</Text><Text style={styles.discoverySummary}>{item.summary}</Text>{item.terms.map(term => <Text key={term} style={styles.infoBody}>• {term}</Text>)}<Pressable testID={`opt-in-${item.id}`} disabled={item.optedIn || !item.eligible} onPress={() => onOptIn(item.id)} style={[styles.secondaryButton, (item.optedIn || !item.eligible) && styles.disabled]}><Text style={styles.secondaryButtonText}>{item.optedIn ? 'JOINED' : 'JOIN DEMO OFFER'}</Text></Pressable></View>) : content.map(item => <Pressable key={item.id} testID={`article-${item.id}`} onPress={() => setArticle(item)} style={styles.discoveryCard}><Text style={styles.gameBadge}>{item.kind.replace('_', ' ').toUpperCase()}</Text><Text style={styles.discoveryTitle}>{item.title}</Text><Text style={styles.discoverySummary}>{item.summary}</Text><Text style={styles.linkText}>READ ARTICLE  ›</Text></Pressable>)}
    {route === 'promotions' ? !promotions.length && <OfferSkeleton /> : !content.length && <OfferSkeleton />}
  </View>;
}
...
const styles = StyleSheet.create({
  ...
  discoveryCard: { marginHorizontal: 10, marginTop: 10, padding: 14, backgroundColor: '#151D28', borderWidth: 1, borderColor: '#263244', gap: 8 },
  articleCard: { margin: 10, padding: 18, backgroundColor: '#151D28', gap: 14, borderLeftWidth: 3, borderLeftColor: '#398DEB' },
  ...
  tableRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderTopWidth: 1, borderTopColor: '#263244', paddingTop: 7 },
